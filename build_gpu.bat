...
rem second arg "wide" builds the 256-bit distance variant
set WIDE=
if /i "%2"=="wide" set WIDE=-DWIDE_DIST
set WIDEC=
if /i "%2"=="wide" set WIDEC=/DWIDE_DIST
set OUTEXE=kangaroo-gpu.exe
if /i "%2"=="wide" set OUTEXE=kangaroo-wide.exe
set OBJD=objgpu
if /i "%2"=="wide" set OBJD=objgpuw
...
"%CUDA_PATH%\bin\nvcc.exe" -maxrregcount=0 --ptxas-options=-v --compile %CCBIN% ^
  -m64 -O2 -I. -I"%CUDA_PATH%\include" -DWITHGPU -DWIN64 %WIDE% -D_CRT_SECURE_NO_WARNINGS ^
  -gencode=arch=compute_%CCAP%,code=sm_%CCAP% ^
  -o %OBJD%\GPUEngine.obj -c GPU\GPUEngine.cu
if errorlevel 1 (echo NVCC FAILED & exit /b 1)

cl /nologo /EHsc /O2 /std:c++14 /DWITHGPU /DWIN64 %WIDEC% /D_CRT_SECURE_NO_WARNINGS ^
  ...
  SECPK1\Point.cpp SECPK1\Random.cpp SECPK1\SECP256K1.cpp /Fo%OBJD%\
if errorlevel 1 (echo COMPILE FAILED & exit /b 1)

link /nologo /OUT:%OUTEXE% %OBJD%\*.obj ws2_32.lib advapi32.lib ^
